...
//poise
//API in old POISE
//under poise/admin/API_ReconcileSKU_poise.php

function mysql_call($query){
    //echo "<BR>" . $query;
    include_once 'db_connect.php';
    
	$db_conn = db_connection('poise');
...
function update_po_items($poData, $param){
	...
	for($i=0; $i < sizeof($poitems); $i++) {
		//if( $poitems[$i]->qty > 0 ) {
			...
			////// Update here Inventory On Order Items
			lock_table('INVENTORY');
			$query = "UPDATE INVENTORY SET ONORDER = ABS(cast(ONORDER AS UNSIGNED) - $diff) WHERE ID = '$oi_invid'";
			$addresult = mysql_call_api($query);
			unlock_tables();				
...
function delete_po_item($poData, $bumsku){
	...
	////// Update here Inventory On Order Items
	lock_table('INVENTORY');
	$query = "UPDATE INVENTORY SET ONORDER = ABS(CAST(ONORDER as SIGNED) - $diff) WHERE ID = '$oi_invid'";
	$addresult = mysql_call_api($query);
	unlock_tables();				
...
function get_po_info_receipt($id){
...
//print_r($poitems);	
	for($i=0; $i < sizeof($poitems); $i++) {
		if( $poitems[$i]->qtytoreceive > 0 ) {
			...
			$query .= "DATERX=NOW(), LASTRXDATE=NOW(), STATUS='$oi_status' ";
			$query .= "WHERE PURCHASEORDERSID='$poId' AND INVENTORYID='$oi_invid'";
//echo $query;
			$result = mysql_call_api($query);
			unlock_tables();
...
function reconcile_inventory($invId){
	$ch = curl_init("http://192.168.1.194/poise/Admin/API_ReconcileSKU_poise.php?");
