...
createPDForder($orderid);
		
		

function createPDForder($orderid){
...
//setup formatting and options
	...
	$orderID = $orderid;//$oitems_list[0]['ORDERID'];	
	
	//$query = "SELECT * FROM ORDERS WHERE ID = '$orderID'";
	//$order_info = mysql_call($query);
	$orderrowID = substr($order_info[0]['ID'],-8);
	$orderdate = date("Y-m-d", strtotime($order_info[0]['CREATIONDATE']));
	$orderPAYMENTTYPE = $invoice_info[0]['PAYMENTTYPE'];
	...
	for($i=0; $i<sizeof($oitems_list); $i++){
		...
		$oidescription[$i] = $oitems_list[$i]['ITEMDESCRIPTION'];
		$oiprice[$i] = $oitems_list[$i]['PRICE'];		
	}


	$orderdiscount = $order_info[0]['DISCOUNTAMOUNT'];

	$orderrowRESTOCKINGFEERATE = $order_info[0]['RESTOCKINGFEERATE'];
	$returnfee = 0;//$invoice_info[0]['RETURNFEE']; 
	$orderrowSHIPCOST = $order_info[0]['SHIPCOST'];
	$orderrowHANDLINGFEE = $order_info[0]['HANDLINGFEE'];
	$SandH = $orderrowSHIPCOST + $orderrowHANDLINGFEE;
	$orderhandlingfee = $order_info[0]['HANDLINGFEE'];
	$HandH = $orderhandlingfee;
	$FormattedSandH = sprintf("%01.2f",$SandH);
	$FormattedHand = sprintf("%01.2f",$HandH);				  
 		    
	$printcod = $order_info[0]['CODFEE'];
	$printcod = sprintf("%1.2f", $printcod);
	
	$dsfee = 0;//$invoice_info[0]['DROPSHIPFEE'];
	...
	$printtotal = $order_info[0]['TOTAL'];
	$printtotal = sprintf("%1.2f", $printtotal);
	
	$totaltax = $order_info[0]['COUNTYTAX'] + $order_info[0]['CITYTAX'] + $order_info[0]['STATETAX'];
	$totaltax = sprintf("%1.2f", $totaltax);

	$ordFirstInitial = strtoupper(substr($employee_info[0]['FIRSTNAME'],0,1));
	$ordLastInitial = strtoupper(substr($employee_info[0]['LASTNAME'],0,1));
	$ordermaker = $ordFirstInitial . $ordLastInitial;
...
$pdf->addInfo('Title',$header['clientname'].' Order #'.$orderid);
$pdf->addInfo('Author','POISE');
$pdf->addinfo('CreationDate',$orderdate);
...
$pdf->ezText('<b>ORDER</b>',22,array('justification'=>'center'));
$pdf->ezText('',10,array('justification'=>'center'));
...
$arrayPayableAccountData2 = array(
     array('C1'=>$ToWhomText1,'C2'=>'<b>ORDER NUMBER:</b>','C3'=>'<b>'.ltrim($orderid,0).'</b>'),
	 array('C1'=>$ToWhomText2,'C2'=>'Invoice Date:','C3'=>$orderdate),
	 array('C1'=>$ToWhomText3,'C2'=>'Terms:','C3'=>$printTerms),
	 array('C1'=>$ToWhomText4,'C2'=>'Due Date:','C3'=>'') 
);
...
$tempsubtotal = $tempsubtotal - $orderdiscount;	  
...
//***********
if ($orderdiscount > 0){
    $arraySubtotalData = array(array('C1'=>'Discount:('.$fdiscount.'%)','C2'=>'('.$orderdiscount.')'));	
...
$pdf->ezText(
	'<b>I='.$ordermaker.', O=<c:alink:'.$root_path.'Print/Order.php?orderid='.$orderrowID.'>'.substr($orderrowID,-8).'</c:alink></b> <b>PS=<c:alink:'.$root_path.'Print/PDFPackingSlip.php?shipmentid='.$shipmentid.'>'.$shipmentid.'</c:alink></b>',
	8,
