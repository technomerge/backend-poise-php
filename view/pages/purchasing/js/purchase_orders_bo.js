...
function Main()
{
	AjaxPopulateSupplierListBo('search', '');
...
function AjaxPopulateSupplierListBo(action, filter_param)
{
	...
	function stateck() 
	{
		if(httpxml.readyState==4)
		{
			...
			$.each(po, function() {
				$dropdown.append($("<option style='font-size:14px' />").val(parseInt(this.SUPPLIER_ID)).text((this.SUPPLIER_INFO).replaceAll('|',String.fromCharCode(9675))));
			});
	...
	url=url+"?sid=" + Math.random() + "&fn=search_supplier_bo" + "&param=" + encodeURIComponent(search_param_str);
...
function AjaxPopulatePoInfo()
{
	...
	function stateck() 
	{
		if(httpxml.readyState==4)
		{
			...
			document.getElementById("po-id").value = '0';//parseInt(po.ID);
			document.getElementById("po-id-static").innerHTML = 'PO #' + '0';//parseInt(po.ID);
			
			document.getElementById("div_select_supplier").style.display = 'none';
			document.getElementById("div_newitem").style.display = 'none';
			
			AjaxPopulatePoItemsBo();
			
//			document.getElementById("poitems_tax").innerHTML = parseFloat(0).toFixed(2);//parseFloat(po.TAX).toFixed(2);
//			document.getElementById("poitems_shipping").innerHTML = parseFloat(0).toFixed(2);//parseFloat(po.SHIPCOST).toFixed(2);
			
			
			AjaxPopulateSupplierInfo(parseInt(document.getElementById("sel-sup").value));
			
			AjaxPopulateBumInfo();
			
			document.getElementById("po-status").value = 'OPEN';//po.STATUS;
			if(po.STATUS == 'CLOSED'){
...
//			AjaxPopulatePoDuedate(parseInt('0'));

			
			document.getElementById("po-employee").value = parseInt(po.EMPLOYEEID);
			AjaxPopulateEmployeesList();
			
			document.getElementById("po-ship-via").value = 'FedEx';//po.SHIPVIA;
			PopulatePoShipViaList();

			document.getElementById("po-payment-type").value = 'NET';//po.PAYMENTTYPE;
...
function AjaxPopulatePoItemsBo()
{
...
$('#poItemsTable tr').not(':first').not(':last').remove();
var html = '';
for(var i = 0; i < poitems.length; i++){
            amt = parseFloat(poitems[i].AMOUNT);
			html += '<tr><td>' + poitems[i].QTY + 
                    '</td><td nowrap>' + poitems[i].SKU + 
					'</td><td nowrap>' + poitems[i].VENDORSKU +
					'</td><td>' + poitems[i].DESCRIPTION +
					'</td><td>' + parseFloat(poitems[i].PRICE).toFixed(2) +
					'</td><td>' + amt.toFixed(2) +
					'</td><td>' + poitems[i].DUEDATE +
					'</td></tr>';
			poitems_subtotal = poitems_subtotal + amt;
}
$('#poItemsTable tr').first().after(html);			
			
			
  $('#poItemsTable').editableTableWidget().numericInputExample().find('td:first').focus();
...
//			document.getElementById("poitems_total").innerHTML = poitems_total.toFixed(2);
	...
	var url = window.location.origin + "/poise/application/controllers/Ajax.php";
	url=url+"?sid=" + Math.random() + "&fn=po_get_items_bo" + "&param=" + encodeURIComponent(supid);	
